...
def require_draft_version_for_manual_edit(session: Session, version_id: int) -> ScheduleVersion:
    ...
    if version.finalized_at is not None:
        raise ManualAssignmentValidationError(
            f"Cannot manually edit finalized schedule version {version_id}. Clone to draft first, then edit the draft."
        )
...
def validate_slot_available(
    session: Session, slot_id: int, schedule_version_id: int, exclude_match_id: Optional[int] = None
    ...
    query = select(MatchAssignment).where(
        MatchAssignment.schedule_version_id == schedule_version_id, MatchAssignment.slot_id == slot_id
    )
...
def validate_duration_fit(match: Match, slot: ScheduleSlot) -> Tuple[bool, Optional[str]]:
    ...
    if match.duration_minutes > slot_minutes:
        return False, (f"Match duration ({match.duration_minutes}min) exceeds slot duration ({slot_minutes}min)")

    max_match_duration = 240
    if match.duration_minutes > max_match_duration:
        return (
            False,
            f"Match duration ({match.duration_minutes}min) exceeds maximum allowed duration ({max_match_duration}min)",
        )
...
def validate_round_dependencies(
    session: Session, match: Match, slot: ScheduleSlot, schedule_version_id: int
) -> Tuple[bool, Optional[str]]:
    """
...
def validate_manual_reassignment(
    session: Session, match_id: int, new_slot_id: int, schedule_version_id: int
) -> Tuple[bool, Optional[str]]:
    """
    ...
    if match.schedule_version_id != schedule_version_id:
        return False, "Match belongs to different schedule version"

    if slot.schedule_version_id != schedule_version_id:
        return False, "Slot belongs to different schedule version"
    ...
    for existing_assignment in court_assignments:
        ...
        if existing_slot and existing_match:
            ...
            if slot_start_minutes < existing_end_minutes and existing_start_minutes < match_end_minutes:
                ...
                min(match_end_minutes, existing_end_minutes)
                overlap_time = time(hour=overlap_start // 60, minute=overlap_start % 60)
                return (
                    False,
                    f"Match would overlap with {existing_match.match_code} on {slot.court_label} starting at {existing_slot.start_time.strftime('%H:%M')}. Overlap at {overlap_time.strftime('%H:%M')}",
                )
    ...
    if tournament_day and tournament_day.end_time:
        ...
        if match_end_minutes > day_end_minutes:
            ...
            return (
                False,
                f"Match would end at {match_end_time.strftime('%H:%M')}, but schedule ends at {tournament_day.end_time.strftime('%H:%M')} on {slot.day_date}",
            )
...
def manually_assign_match(
    session: Session, match_id: int, new_slot_id: int, schedule_version_id: int, assigned_by: str = "MANUAL"
) -> MatchAssignment:
    """
    ...
    if version.status != "draft":
        raise ManualAssignmentValidationError(
            f"Cannot modify assignments on {version.status} schedule versions. Clone to draft first to make changes."
        )
    ...
    existing = session.exec(
        select(MatchAssignment).where(
            MatchAssignment.schedule_version_id == schedule_version_id, MatchAssignment.match_id == match_id
        )
    ).first()
