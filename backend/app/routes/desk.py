...
def _build_checkin_snapshot(
    ...
    checkin_board_courts = all_desk_courts(
        tournament.court_names or [],
        grid_slots,
        extra_courts=active_courts,
    )
    ...
    # Current block first, then any later Grid cell so every empty court can show.
    ...
    for s in grid_slots:
        if current_board_key and slot_key_for_slot(s) < current_board_key:
            continue
        _append_available_slot(s)
...
def assign_ready_match_to_slot(
    ...
    if target_assignment and target_assignment.match_id != payload.match_id:
        ...
        park_slot_id: Optional[int] = None
        selected_old_slot_id = selected_assignment.slot_id
        if selected_old_slot_id and selected_old_slot_id != effective_slot_id:
            park_slot_id = selected_old_slot_id
        else:
            all_assignments = session.exec(
                select(MatchAssignment).where(
                    MatchAssignment.schedule_version_id == payload.version_id,
