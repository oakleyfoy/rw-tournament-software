...
def test_locked_assignments_not_moved_by_autoassign(client: TestClient, session: Session, manual_editor_setup):
    ...
    assignment_in_db = session.exec(
        select(MatchAssignment).where(
            MatchAssignment.match_id == first_assignment["match_id"], MatchAssignment.schedule_version_id == version_id
        )
    ).first()
    ...
    sum(1 for a in all_assignments_after if not a.locked)
...
def test_manual_move_enforces_duration_fit(client: TestClient, session: Session, manual_editor_setup):
    ...
    manual_resp = client.patch(
        f"/api/tournaments/{tournament_id}/schedule/assignments/{assignment.id}", json={"new_slot_id": slot.id}
    )
...
def test_manual_move_enforces_slot_availability(client: TestClient, session: Session, manual_editor_setup):
    ...
    other_slot = session.exec(
        select(ScheduleSlot).where(ScheduleSlot.schedule_version_id == version_id, ScheduleSlot.id != slot.id)
    ).first()
    ...
    manual_resp = client.patch(
        f"/api/tournaments/{tournament_id}/schedule/assignments/{assignment2.id}", json={"new_slot_id": slot.id}
    )
...
def test_manual_move_fails_on_finalized_version(client: TestClient, session: Session, manual_editor_setup):
    ...
    manual_resp = client.patch(
        f"/api/tournaments/{tournament_id}/schedule/assignments/{assignment.id}", json={"new_slot_id": other_slot.id}
    )
...
def test_clone_preserves_locked_assignments(client: TestClient, session: Session, manual_editor_setup):
    ...
    source_slots = session.exec(select(ScheduleSlot).where(ScheduleSlot.schedule_version_id == version_id)).all()
    cloned_slots = session.exec(
        select(ScheduleSlot).where(ScheduleSlot.schedule_version_id == cloned_version["id"])
    ).all()

    source_matches = session.exec(select(Match).where(Match.schedule_version_id == version_id)).all()
    cloned_matches = session.exec(select(Match).where(Match.schedule_version_id == cloned_version["id"])).all()
    ...
    {slot_key(s): s.id for s in source_slots}
    {slot_key(s): s.id for s in cloned_slots}

    {match_key(m): m.id for m in source_matches}
    {match_key(m): m.id for m in cloned_matches}
    ...
    for a in pre_finalize_assignments:
        ...
        if source_slot and source_match:
            source_assignment_tuples.append((match_key(source_match), slot_key(source_slot), a.locked, a.assigned_by))
    ...
    for a in cloned_assignments:
        ...
        if cloned_slot and cloned_match:
            cloned_assignment_tuples.append((match_key(cloned_match), slot_key(cloned_slot), a.locked, a.assigned_by))
...
def test_successful_manual_reassignment(client: TestClient, session: Session, manual_editor_setup):
    ...
    manual_resp = client.patch(
        f"/api/tournaments/{tournament_id}/schedule/assignments/{assignment.id}", json={"new_slot_id": slot2.id}
    )
...
def test_manual_reassignment_returns_enriched_response(client: TestClient, session: Session, manual_editor_setup):
    ...
    # Manually reassign
    resp = client.patch(
        f"/api/tournaments/{tournament_id}/schedule/assignments/{assignment.id}", json={"new_slot_id": slots[1].id}
    )
