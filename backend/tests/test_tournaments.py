...
def test_start_over_uses_latest_final_layout_when_public_pointer_missing(
    ...
    # Explicitly clear pointer to validate latest-final fallback path.
    source.public_schedule_version_id = None
    session.add(source)
    session.commit()
    ...
    assert dup_assignment is not None
    assert dup_assignment.slot_id == dup_slot_by_day[date(2026, 9, 1)].id

    restart_resp = client.post(f"/api/tournaments/{duplicated_id}/start-over")
    assert restart_resp.status_code == 200

    session.refresh(dup_assignment)
    assert dup_assignment.slot_id == dup_slot_by_day[date(2026, 9, 3)].id
