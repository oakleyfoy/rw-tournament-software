...
def create_tournament(session: Session) -> int:
...
def test_finalize_draft_version(client: TestClient, session: Session):
    """Test that finalize sets checksum and locks version"""
    tid = create_tournament(session)
...
def test_mutation_rejected_on_final_version(client: TestClient, session: Session):
...
def test_clone_final_to_draft(client: TestClient, session: Session):
...
def test_checksum_determinism(client: TestClient, session: Session):
...
def test_clone_only_works_on_final(client: TestClient, session: Session):
...
def test_clone_to_draft_alias_route(client: TestClient, session: Session):
...
def test_reset_draft_version(client: TestClient, session: Session):
